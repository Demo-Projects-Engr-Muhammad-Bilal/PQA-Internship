...
import { Menu, LogOut, Settings, FileText, Users } from "lucide-react";
...
function Header({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  const getBreadcrumbs = () => {
    ...
    if (segments.includes("pilots")) {
      breadcrumbs.push({ label: "Pilots", href: "/dashboard/pilots" });
    } else if (segments.includes("forms") && segments.length > 2) {
      breadcrumbs.push({ label: "Forms", href: "/dashboard" });
      breadcrumbs.push({ label: "Review", href: pathname });
    }
          ...
          <SheetContent side="left" className="w-64 p-0">
            <div className="flex h-full flex-col">
              <div className="border-b px-4 h-16 flex items-center justify-start">
                  <Image src="/logo-actual.png" alt="PQA Logo" width={200} height={60} className="w-auto h-12 object-contain" />
              </div>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <SidebarMenuButton className="group-data-[collapsible=icon]:!justify-center group-data-[collapsible=icon]:!px-0">
                        ...
                        <span className="truncate text-sm group-data-[collapsible=icon]:hidden">Admin User</span>
                      </SidebarMenuButton>
                    </DropdownMenuTrigger>
